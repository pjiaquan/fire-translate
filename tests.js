...
// Load scripts
const utilsDomainCode = fs.readFileSync('utils/domain.js', 'utf8');
const popupCode = utilsDomainCode + "\n" + fs.readFileSync('popup.js', 'utf8');
const bgCode = fs.readFileSync('background.js', 'utf8');
const contentCode = utilsDomainCode + "\n" + fs.readFileSync('content.js', 'utf8');
...
async function executeTestSuite() {
  ...
  await runTest("Website domain exclusion should mute translation when domain or base domain (*.example.com) is disabled", () => {
    const sandbox = createSandbox();
    vm.createContext(sandbox);
    vm.runInContext(utilsDomainCode, sandbox);
